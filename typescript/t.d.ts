 ...
 ******************************************************** */
// ParseInterpolationValues was modified to add stricter typechecking when the format requires a
// specific type. This enforces it for a few common format types.
//
// With this change:
//
// - `t('{{val, number}}', { val: 123 })` will typecheck
// - `t('{{val, number}}', { val: 'myStr' })` will fail to typecheck
...
type InterpolationMap<Ret> =
  ...
  // Keep the structure of the string after `extends` the same as the structure of the string in
  // `TFunction`.
